...
export async function renderDefaultersReportView() {
    ...
    // Formata o mês para exibição no título (Ex: "Outubro de 2025")
    const date = new Date(defaultersReportMonth + '-02T00:00:00'); // Usar o dia 2 evita bugs de fuso horário
    ...
    return `
        ...
        <div class="card full-width">
            ...
            <div class="dashboard-controls" style="justify-content: space-between;">
                <div class="dashboard-controls">
                    ...
                    <button class="month-nav-button" onclick="window.handleDefaultersReportChangeMonth(-1)">‹</button>
                    <input type="month" id="defaulters-month-selector" name="month" value="${defaultersReportMonth}" onchange="window.handleDefaultersReportDateChange(event)">
                    <button class="month-nav-button" onclick="window.handleDefaultersReportChangeMonth(1)">›</button>
                </div>
                <div class="dashboard-controls">
                     <label for="defaulters-course-selector">Filtrar por Curso:</label>
                     <select id="defaulters-course-selector" onchange="window.handleDefaultersReportCourseChange(event)">
        ...
        <form class="card full-width" onsubmit="window.handleBulkPay(event)">
                        ...
                        ${defaulters.length === 0 ? '<tr><td colspan="8" style="text-align: center;">Nenhuma pendência encontrada para este filtro.</td></tr>' : defaulters.map(item => {
                            ...
                            return `
                                <tr>
                                    ...
                                    <td>
                                        <button type="button" class="action-button secondary" onclick="window.handleNavigateToProfile(${item.studentId})">Ver Perfil</button>
                                    </td>
                                </tr>
                            `;
                        }).join('')}
