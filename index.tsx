...
interface User {
  ...
  lastName?: string;
  email: string;
  password?: string; // Password should not be stored on the client
  role: UserRole;
  age?: number;
...
declare global {
    interface Window {
        ...
        handleLogout: () => void;
        navigateTo: (view: 'login' | 'register') => void;
        handleEnroll: (event: Event) => Promise<void>;
        handleApprove: (event: Event) => Promise<void>;
        handleSaveAttendance: (event: Event) => Promise<void>;
        handleCreateCourse: (event: Event) => Promise<void>;
        handleRoleChange: (event: Event, userId: number) => Promise<void>;
        handleEndCourse: (courseId: number) => Promise<void>;
        handleReopenCourse: (courseId: number) => Promise<void>;
        handleNavigateToAttendance: (courseId: number) => Promise<void>;
        handleNavigateBackToDashboard: () => void;
        handleNavigateToUserManagement: () => Promise<void>;
        handleUserFilterChange: (event: Event) => void;
        handleNavigateToEditCourse: (courseId: number) => Promise<void>;
        handleUpdateCourse: (event: Event) => Promise<void>;
        handleNavigateToCourseDetails: (courseId: number) => Promise<void>;
        handleNavigateToProfile: (userId: number) => Promise<void>;
        handleNavigateToSchoolProfile: () => Promise<void>;
        ...
        handleNavigateToFinancialDashboard: () => Promise<void>;
        handlePaymentStatusChange: (event: Event, paymentId: number) => Promise<void>;
        handleNavigateToFinancialControlPanel: () => Promise<void>;
        handleToggleFinanceStudent: (studentId: number) => Promise<void>;
        ...
        handleNavigateToSystemSettings: () => Promise<void>;
        handleUpdateSystemSettings: (event: Event) => Promise<void>;
...
async function apiCall(action: string, data: Record<string, any> = {}, method: 'GET' | 'POST' = 'POST') {
    try {
...
const appState = {
    ...
    currentView: 'login' as 'login' | 'register' | 'dashboard',
    ...
    // UI State
    // FIX: Expanded adminView type to include 'editCourse' and 'details'
    adminView: 'dashboard' as 'dashboard' | 'userManagement' | 'systemSettings' | 'createCourse' | 'editCourse' | 'details',
    viewingCourseId: null as number | null,
    viewingUserId: null as number | null,
    ...
    attendanceState: {
        ...
        students: [] as User[],
        history: {} as Record<string, AttendanceRecord[]>,
    },
    financialState: {
    ...
    pixModal: {
        ...
        content: null as { qrCodeUrl: string; pixCode: string; totalAmount: number; coursesInfo: string; } | null,
    }
};
...
async function render() {
  ...
  // Logged-in views
  const { financialState, attendanceState, viewingCourseId, viewingUserId, adminView } = appState;
  ...
  if (attendanceState.courseId !== null) {
      appRoot.innerHTML = await renderAttendanceManagementView(attendanceState.courseId);
      return;
  }
...
function renderStudentView(studentId: number, data: any) {
  ...
  const cards = [
    {
      ...
      html: `
        <div class="card" id="student-courses" draggable="true" ondragstart="window.handleDragStart(event)" ondragend="window.handleDragEnd(event)">
            ...
            <ul class="list">
                ${myEnrollments.map((enrollment: Enrollment) => {
                    const course = openCourses.find((c: Course) => c.id === enrollment.courseId);
                    if (!course) return '';
                    const teacher = data.teachers.find((t: User) => t.id === course.teacherId);
            ...
            <h3 class="card-title">📊 Meu Relatório de Frequência</h3>
             ${myAttendance.length === 0 ? '<p>Nenhum registro de frequência ainda.</p>' : `
                ...
                <table>
                    ...
                    <tbody>
                        ${myAttendance.map((record: AttendanceRecord) => {
                            const course = openCourses.find((c: Course) => c.id === record.courseId);
...
function renderAdminView(adminId: number, data: any) {
    const admin = appState.currentUser;
    if (!admin) return '';

    const pendingEnrollments = data.enrollments.filter((e: Enrollment) => e.status === 'Pendente');
    const allCourses = data.courses || [];
    const openCourses = allCourses.filter((c: Course) => c.status === 'Aberto');
        ...
        },
        {
          ...
          html: `
            <div class="card" id="admin-pending-enrollments" draggable="true" ondragstart="window.handleDragStart(event)" ondragend="window.handleDragEnd(event)">
                ...
                <ul class="list">
                    ${pendingEnrollments.length === 0 ? '<li>Nenhuma matrícula pendente.</li>' : pendingEnrollments.map((enrollment: Enrollment) => {
                        const student = data.users.find((s: User) => s.id === enrollment.studentId);
                        const course = allCourses.find((c: Course) => c.id === enrollment.courseId);
                        if (!student || !course) return '';
                        const enrolledCount = data.enrollments.filter((e: Enrollment) => e.courseId === course.id && e.status === 'Aprovada').length;
        ...
        },
        {
          ...
          html: `
            <div class="card" id="admin-attendance" draggable="true" ondragstart="window.handleDragStart(event)" ondragend="window.handleDragEnd(event)">
                ...
                <ul class="list">
                    ${openCourses.length === 0 ? '<li>Nenhum curso aberto.</li>' : openCourses.map((course: Course) => `
                        <li class="list-item">
                             <div class="list-item-content">
                                <span class="list-item-title">${course.name}</span>
                            </div>
...
async function renderUserManagementView(adminId: number) {
...
function renderTeacherView(teacherId: number, data: any) {
    const teacher = appState.currentUser;
    if (!teacher) return '';

    const myOpenCourses = data.courses || [];

    const cards = myOpenCourses.map((course: Course) => ({
    ...
    return `
        ...
        <div class="dashboard-grid" ondragover="window.handleDragOver(event)" ondrop="window.handleDrop(event)">
            ${cards.length === 0 ? '<div class="card"><p>Nenhum curso aberto atribuído a você.</p></div>' : cards.map((c: any) => c.html).join('')}
        </div>
    `;
}

async function renderAttendanceManagementView(courseId: number) {
                    ...
                    <form onsubmit="window.handleSaveAttendance(event)" data-course-id="${course.id}">
                        ...
                        <table>
                            ...
                            ${data.students.map((student: User) => {
                                const existingRecord = existingRecordsForDate.find((r: AttendanceRecord) => r.studentId === student.id);
                ...
                ${Object.keys(attendanceHistory).length === 0 ? '<p>Nenhum histórico de frequência.</p>' : `
                 <div class="table-wrapper">
                    <table>
                        ...
                        <tbody>
                            ${Object.entries(attendanceHistory).sort(([dateA], [dateB]) => dateB.localeCompare(dateA)).map(([date, summary]: [string, any]) => {
...
async function renderEditCourseView(course: Course) {
                ...
                <div class="form-group">
                 ...
                 <div class="form-group">
                    ...
                    <select id="editTeacherId" name="teacherId" required>
                        ${teachers.map((t: User) => `<option value="${t.id}" ${t.id === course.teacherId ? 'selected' : ''}>${t.firstName} ${t.lastName || ''}</option>`).join('')}
                    </select>
                </div>
                <div class="form-group">
                    <label for="totalSlots">Número de Vagas</label>
...
async function renderCourseDetailsView(course: Course) {
...
async function renderProfileView(userId: number) {
    ...
    return `
        ...
        <div class="card full-width">
            ...
            <ul class="list finance-student-list">
                ${activeStudents.map((student: User) => `
...
function renderStudentFinancialHistory(studentId: number, studentPayments: Payment[], isAdminView = false, allowActions = false) {
...
window.navigateTo = (view: 'login' | 'register') => {
...
window.handleEnroll = async (event: Event) => {
...
window.handleApprove = async (event: Event) => {
    event.preventDefault();
    const form = event.target as HTMLFormElement;
...
window.handleSaveAttendance = async (event: Event) => {
    event.preventDefault();
    const form = event.target as HTMLFormElement;
    const courseId = parseInt(form.dataset.courseId!, 10);
...
window.handleCreateCourse = async (event: Event) => {
    event.preventDefault();
    const form = event.target as HTMLFormElement;
    const formData = new FormData(form);
...
window.handleUpdateCourse = async (event: Event) => {
    event.preventDefault();
    const form = event.target as HTMLFormElement;
    const formData = new FormData(form);
    const courseData = Object.fromEntries(formData.entries());
...
window.handleUpdateSchoolProfile = async (event: Event) => {
    ...
    const profileData: Record<string, any> = Object.fromEntries(formData.entries());
...
window.handleUpdateSystemSettings = async (event: Event) => {
    event.preventDefault();
    const form = event.target as HTMLFormElement;
    const formData = new FormData(form);
...
window.handleRoleChange = async (event: Event, userId: number) => {
...
window.handleUserFilterChange = (event: Event) => {
    ...
    type FilterKeys = keyof typeof appState.userFilters;
    if (name === 'courseId') {
        ...
        (appState.userFilters as any)[name as FilterKeys] = value;
    }
    render();
}
...
window.handleEndCourse = async (courseId: number) => {
...
window.handleReopenCourse = async (courseId: number) => {
...
window.handleNavigateToAttendance = async (courseId: number) => {
    appState.attendanceState.courseId = courseId;
    appState.attendanceState.selectedDate = new Date().toISOString().split('T')[0];
    render();
}

window.handleNavigateToEditCourse = async (courseId: number) => {
...
window.handleNavigateToCourseDetails = async (courseId: number) => {
    appState.viewingCourseId = courseId;
...
window.handleNavigateToProfile = async (userId: number) => {
...
window.handleToggleFinanceStudent = async (studentId: number) => {
...
window.handlePaymentStatusChange = async (event: Event, paymentId: number) => {
    const select = event.target as HTMLSelectElement;
