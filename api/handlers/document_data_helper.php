...
function get_document_details(PDO $conn, $studentId, $courseId) {
    ...
    $sql = " SELECT u.id as studentId, u.firstName, u.lastName, u.email, u.rg as aluno_rg, u.cpf as aluno_cpf, u.address as aluno_endereco, u.age, u.guardianName, u.guardianRG, u.guardianCPF, u.guardianEmail, u.guardianPhone, c.id as courseId, c.name as courseName, c.monthlyFee, s.name as schoolName, s.cnpj as schoolCnpj, s.address as schoolAddress, s.profilePicture, s.signatureImage, st.enrollmentContractText, st.imageTermsText, st.defaultDueDay, e.contractAcceptedAt, e.termsAcceptedAt FROM users u LEFT JOIN enrollments e ON u.id = e.studentId AND e.courseId = :courseId_e JOIN courses c ON c.id = :courseId_c JOIN system_settings st ON st.id = 1 JOIN school_profile s ON s.id = 1 WHERE u.id = :studentId LIMIT 1 ";
    try {
