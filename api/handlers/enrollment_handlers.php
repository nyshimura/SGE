...
function handle_approve_enrollment(PDO $conn, $data) {
    ...
    try {
        ...
        if ($stmt->rowCount() == 0) { /* ... (lógica de verificação se já aprovada) ... */ $conn->rollBack(); $cStmt=$conn->prepare("SELECT status FROM enrollments WHERE studentId=? AND courseId=?"); $cStmt->execute([$studentId,$courseId]); $cS=$cStmt->fetchColumn(); if($cS==='Aprovada'){send_response(true,['message'=>'Matrícula já aprovada.']);}else{throw new Exception('Matrícula não pendente.');} return; }
        ...
        // --- ENVIO DE E-MAIL ---
        $emailSent = false;
        $responseMessage = 'Matrícula aprovada e pagamentos gerados.'; // Mensagem padrão

        if (!empty($emailData['toEmail'])) {
            // Busca o nome da escola e os modelos de e-mail
            $stmtMailSettings = $conn->query("SELECT s.name as schoolName, ss.site_url, ss.email_approval_subject, ss.email_approval_body, ss.smtpUser 
            ...
            if ($mailSettings && !empty($mailSettings['smtpUser']) && !empty($mailSettings['email_approval_subject']) && !empty($mailSettings['email_approval_body'])) {
                ...
                // Cria o link do contrato
                $siteUrl = rtrim($mailSettings['site_url'] ?? '', '/'); // Remove barra final se houver
                $contractLink = $siteUrl . "/api/index.php?action=generateContractPdf&studentId=$studentId&courseId=$courseId";

                // Placeholders
                $placeholders = [
                    '{{aluno_nome}}' => htmlspecialchars($emailData['studentFirstName']), // Nome do Aluno
                    '{{responsavel_nome}}' => htmlspecialchars($emailData['toName']), // Nome do Destinatário (aluno ou responsável)
                    '{{curso_nome}}' => htmlspecialchars($emailData['courseName']),
                    '{{escola_nome}}' => htmlspecialchars($mailSettings['schoolName']),
                    '{{link_contrato}}' => $contractLink // Link direto para o PDF
                ];

                // Substitui placeholders no Assunto e Corpo
                $subject = $mailSettings['email_approval_subject'];
                $bodyHTML = nl2br($mailSettings['email_approval_body']); // Converte \n para <br>
                ...
                if ($emailSent) {
                    $responseMessage .= ' E-mail de confirmação enviado para ' . $emailData['toEmail'] . '.';
                } else {
                    $responseMessage .= ' Falha ao enviar e-mail de confirmação (verifique config. SMTP).';
                }
            } else {
...
// ... (Funções handle_cancel_enrollment, handle_reactivate_enrollment, handle_update_enrollment_details - mantidas) ...
function handle_cancel_enrollment($conn, $data) { /* ... (código mantido) ... */ $sId=isset($data['studentId'])?(int)$data['studentId']:null;$cId=isset($data['courseId'])?(int)$data['courseId']:null;if(!$sId||!$cId){send_response(false,'IDs obrigatórios.',400);}$conn->beginTransaction();try{$stmt=$conn->prepare("UPDATE enrollments SET status='Cancelada' WHERE studentId=? AND courseId=? AND status='Aprovada'");$stmt->execute([$sId,$cId]);if($stmt->rowCount()==0){throw new Exception('Matrícula não aprovada.');}$setStmt=$conn->query("SELECT enableTerminationFine,terminationFineMonths FROM system_settings WHERE id=1");$set=$setStmt->fetch(PDO::FETCH_ASSOC);$fineE=isset($set['enableTerminationFine'])&&$set['enableTerminationFine']==1;$fineM=isset($set['terminationFineMonths'])?(int)$set['terminationFineMonths']:0;$msg='Matrícula trancada.';$payStmt=$conn->prepare("SELECT id FROM payments WHERE studentId=? AND courseId=? AND status IN ('Pendente','Atrasado') ORDER BY dueDate ASC");$payStmt->execute([$sId,$cId]);$fPay=$payStmt->fetchAll(PDO::FETCH_COLUMN);$toCancel=[];if(count($fPay)>0){if($fineE&&$fineM>0){$toCancel=array_slice($fPay,$fineM);$kept=count($fPay)-count($toCancel);if($kept>0)$msg.=" Multa de {$kept} mensalidade(s) mantida.";if(!empty($toCancel))$msg.=" Demais cancelados.";}else{$toCancel=$fPay;$msg.=' Pagamentos futuros cancelados.';}if(!empty($toCancel)){$phs=implode(',',array_fill(0,count($toCancel),'?'));$canSql="UPDATE payments SET status='Cancelado',paymentDate=NULL WHERE id IN ($phs)";$canStmt=$conn->prepare($canSql);$canStmt->execute($toCancel);}}else{$msg.=' Sem pagamentos futuros.';}$conn->commit();send_response(true,['message'=>$msg]);}catch(Exception $e){$conn->rollBack();error_log("Erro trancar: ".$e->getMessage());send_response(false,'Erro trancar: '.$e->getMessage(),500);} }
function handle_reactivate_enrollment($conn, $data) { /* ... (código mantido) ... */ $sId=isset($data['studentId'])?(int)$data['studentId']:null;$cId=isset($data['courseId'])?(int)$data['courseId']:null;if(!$sId||!$cId){send_response(false,'IDs obrigatórios.',400);}$conn->beginTransaction();try{$stmt=$conn->prepare("UPDATE enrollments SET status='Aprovada' WHERE studentId=? AND courseId=? AND status='Cancelada'");$stmt->execute([$sId,$cId]);if($stmt->rowCount()==0){throw new Exception('Matrícula não cancelada.');}$payStmt=$conn->prepare("SELECT id,dueDate FROM payments WHERE studentId=? AND courseId=? AND status='Cancelado'");$payStmt->execute([$sId,$cId]);$canPay=$payStmt->fetchAll(PDO::FETCH_ASSOC);$today=new DateTime('now',new DateTimeZone('America/Sao_Paulo'));$today->setTime(0,0,0);foreach($canPay as $p){$due=new DateTime($p['dueDate'],new DateTimeZone('America/Sao_Paulo'));$due->setTime(0,0,0);$newS=($due<$today)?'Atrasado':'Pendente';$upP=$conn->prepare("UPDATE payments SET status=? WHERE id=?");$upP->execute([$newS,$p['id']]);}$conn->commit();send_response(true,['message'=>'Matrícula reativada. Pagamentos restaurados.']);}catch(Exception $e){$conn->rollBack();error_log("Erro reativar: ".$e->getMessage());send_response(false,'Erro reativar: '.$e->getMessage(),500);} }
function handle_update_enrollment_details($conn, $data) { /* ... (código mantido) ... */ $sId=filter_var($data['studentId'],FILTER_VALIDATE_INT);$cId=filter_var($data['courseId'],FILTER_VALIDATE_INT);$sch=isset($data['scholarshipPercentage'])&&is_numeric($data['scholarshipPercentage'])?(float)$data['scholarshipPercentage']:0.0;if($sch<0||$sch>100)$sch=0.0;$cFee=isset($data['customMonthlyFee'])&&$data['customMonthlyFee']!==''&&is_numeric($data['customMonthlyFee'])&&(float)$data['customMonthlyFee']>=0?(float)$data['customMonthlyFee']:null;if($sId===false||$cId===false){send_response(false,'IDs inválidos.',400);}$conn->beginTransaction();try{$upStmt=$conn->prepare("UPDATE enrollments SET scholarshipPercentage=?,customMonthlyFee=? WHERE studentId=? AND courseId=?");$upStmt->execute([$sch,$cFee,$sId,$cId]);$delStmt=$conn->prepare("DELETE FROM payments WHERE studentId=? AND courseId=? AND status IN ('Pendente','Atrasado')");$delStmt->execute([$sId,$cId]);$stD=$conn->prepare("SELECT c.monthlyFee,c.paymentType,c.installments,e.billingStartDate FROM courses c JOIN enrollments e ON c.id=e.courseId WHERE e.studentId=? AND e.courseId=? AND e.status='Aprovada'");$stD->execute([$sId,$cId]);$d=$stD->fetch();if($d){$dueDay=$conn->query("SELECT defaultDueDay FROM system_settings WHERE id=1")->fetchColumn()??10;$dueDay=max(1,min(28,(int)$dueDay));if($sch<100){$bA=$cFee!==null?$cFee:($d['monthlyFee']??0);$fA=round($bA*(1-($sch/100)),2);if($fA>0){$lastP=$conn->prepare("SELECT MAX(referenceDate) as lastDate FROM payments WHERE studentId=? AND courseId=? AND status='Pago'");$lastP->execute([$sId,$cId]);$lastPD=$lastP->fetchColumn();$startD=null;if($lastPD){$startD=new DateTime($lastPD,new DateTimeZone('America/Sao_Paulo'));$startD->modify('first day of next month');}elseif($d['billingStartDate']){$startD=new DateTime($d['billingStartDate'],new DateTimeZone('America/SaoPaulo'));$startD->modify('first day of this month');$nowD=new DateTime('now',new DateTimeZone('America/Sao_Paulo'));if($startD<$nowD){$startD=$nowD;$startD->modify('first day of this month');}}if($startD){$paidC=$conn->prepare("SELECT COUNT(id) FROM payments WHERE studentId=? AND courseId=? AND status='Pago'");$paidC->execute([$sId,$cId]);$pMadeC=$paidC->fetchColumn();$limit=0;if($d['paymentType']==='parcelado'&&!empty($d['installments'])&&$d['installments']>0){$rem=(int)$d['installments']-$pMadeC;$limit=max(0,$rem);}else{$limit=12;}$curD=clone $startD;for($i=0;$i<$limit;$i++){$refD=$curD->format('Y-m-01');$lastDay=(int)$curD->format('t');$actDue=min($dueDay,$lastDay);$dueD=$curD->format('Y-m-').str_pad($actDue,2,'0',STR_PAD_LEFT);$insStmt=$conn->prepare("INSERT INTO payments(studentId,courseId,amount,referenceDate,dueDate,status)VALUES(?,?,?,?,?,'Pendente')");$insStmt->execute([$sId,$cId,$fA,$refD,$dueD]);$curD->modify('+1 month');}}else{error_log("Falha startDate $sId,$cId");}}}} $conn->commit();send_response(true,['message'=>'Detalhes atualizados. Pagamentos recalculados.']);}catch(Exception $e){$conn->rollBack();error_log("Erro updateEnrollDet: ".$e->getMessage());send_response(false,'Erro ao atualizar: '.$e->getMessage(),500);} }

// --- REMATRÍCULA AUTOMÁTICA ---
function handle_submit_reenrollment(PDO $conn, $data) { /* ... (código mantido) ... */ $sId=isset($data['studentId'])?filter_var($data['studentId'],FILTER_VALIDATE_INT):0;$cId=isset($data['courseId'])?filter_var($data['courseId'],FILTER_VALIDATE_INT):0;$eData=$data['enrollmentData']??[];if($sId<=0||$cId<=0){send_response(false,'IDs inválidos.',400);}if(empty($eData['acceptContract'])){send_response(false,'Aceite o Contrato.',400);}$conn->beginTransaction();try{$stC=$conn->prepare("SELECT id FROM enrollments WHERE studentId=? AND courseId=? AND status='Aprovada'");$stC->execute([$sId,$cId]);$eId=$stC->fetchColumn();if(!$eId){throw new Exception('Matrícula não ativa.');}$now=date('Y-m-d H:i:s');$cA=$now;$tA=!empty($eData['acceptImageTerms'])?$now:null;$upE=$conn->prepare("UPDATE enrollments SET contractAcceptedAt=?,termsAcceptedAt=? WHERE id=?");$upE->execute([$cA,$tA,$eId]);$delS=$conn->prepare("DELETE FROM payments WHERE studentId=? AND courseId=? AND status IN('Pendente','Atrasado')");$delS->execute([$sId,$cId]);$delC=$delS->rowCount();$stD=$conn->prepare("SELECT c.monthlyFee,c.paymentType,c.installments,e.customMonthlyFee,e.scholarshipPercentage FROM courses c JOIN enrollments e ON c.id=e.courseId WHERE e.id=?");$stD->execute([$eId]);$d=$stD->fetch();$stS=$conn->query("SELECT defaultDueDay FROM system_settings WHERE id=1");$set=$stS->fetch();$dueD=isset($set['defaultDueDay'])?max(1,min(28,(int)$set['defaultDueDay'])):10;$genC=0;if($d&&($d['monthlyFee']>0||$d['customMonthlyFee']!==null)&&($d['scholarshipPercentage']===null||$d['scholarshipPercentage']<100)){$bA=$d['customMonthlyFee']!==null?$d['customMonthlyFee']:$d['monthlyFee'];$sch=$d['scholarshipPercentage']??0;$fA=round($bA*(1-($sch/100)),2);if($fA>0){$lastP=$conn->prepare("SELECT MAX(referenceDate) as lastDate FROM payments WHERE studentId=? AND courseId=?");$lastP->execute([$sId,$cId]);$lastPD=$lastP->fetchColumn();$startD=new DateTime('now',new DateTimeZone('America/Sao_Paulo'));if($lastPD){$startD=new DateTime($lastPD,new DateTimeZone('America/Sao_Paulo'));$startD->modify('first day of next month');}else{$bS=$conn->prepare("SELECT billingStartDate FROM enrollments WHERE id=?");$bS->execute([$eId]);$bSD=$bS->fetchColumn();if($bSD){$startD=new DateTime($bSD,new DateTimeZone('America/Sao_Paulo'));$startD->modify('first day of this month');$nowD=new DateTime('now',new DateTimeZone('America/Sao_Paulo'));if($startD<$nowD){$startD=$nowD;$startD->modify('first day of this month');}}else{$startD->modify('first day of this month');}}$limit=0;if($d['paymentType']==='parcelado'&&!empty($d['installments'])&&$d['installments']>0){$pC=$conn->prepare("SELECT COUNT(id) FROM payments WHERE studentId=? AND courseId=? AND status='Pago'");$pC->execute([$sId,$cId]);$pMadeC=$pC->fetchColumn();$limit=max(0,(int)$d['installments']-$pMadeC);}else{$limit=12;}$curD=clone $startD;for($i=0;$i<$limit;$i++){$refD=$curD->format('Y-m-01');$lastDay=(int)$curD->format('t');$actDue=min($dueD,$lastDay);$dueD=$curD->format('Y-m-').str_pad($actDue,2,'0',STR_PAD_LEFT);$insStmt=$conn->prepare("INSERT INTO payments(studentId,courseId,amount,referenceDate,dueDate,status)VALUES(?,?,?,?,?,'Pendente')");$insStmt->execute([$sId,$cId,$fA,$refD,$dueD]);$genC++;$curD->modify('+1 month');}}}$conn->commit();send_response(true,['message'=>"Rematrícula confirmada! ".($genC>0?"$genC pagamentos gerados.":"Nenhum gerado.").($delC>0?" $delC anteriores removidos.":"")]);}catch(Exception $e){$conn->rollBack();error_log("Erro submitReenr: ".$e->getMessage());send_response(false,'Erro rematrícula: '.$e->getMessage(),500);} }
...
function handle_get_enrollment_documents(PDO $conn, $data) {
    ...
    try {
        ...
        // 4. SUBSTITUIÇÃO DOS PLACEHOLDERS (Correção principal)
        ...
        // Nota: nl2br e htmlspecialchars são usados para formatar o texto substituído para exibição HTML.
        send_response(true, [
