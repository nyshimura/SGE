...
// FIX: Aligned global window interface with index.tsx to resolve type conflicts.
declare global {
    interface Window {
...
const db = {
  users: [
    { id: 1, firstName: 'Super', lastName: 'Admin', email: 'admin@admin', password: 'admin', role: 'superadmin' as UserRole },
    { id: 2, firstName: 'Ana', lastName: 'Silva', email: 'ana@email.com', password: '123', role: 'student' as UserRole },
    { id: 3, firstName: 'Marcos', lastName: 'Costa', email: 'marcos@email.com', password: '123', role: 'student' as UserRole },
    { id: 4, firstName: 'Carlos', lastName: 'Gerente', email: 'carlos@email.com', password: '123', role: 'admin' as UserRole },
    { id: 5, firstName: 'Prof.', lastName: 'Silva', email: 'silva@email.com', password: '123', role: 'teacher' as UserRole },
  ] as User[],
  courses: [
  ...
  schoolProfile: {
      name: 'Rafael Nishimura Abreu',
      cnpj: '12.345.678/0001-99',
      address: 'Rua da Imaginação, 123, Centro',
      phone: '(11) 98765-4321',
...
let currentView: 'login' | 'register' | 'dashboard' = 'login';
...
// FIX: Made function async to match global type definition.
...
window.handleUpdateProfile = async (event: Event) => {
    ...
    if (userToUpdate) {
        userToUpdate.firstName = formData.get('firstName') as string || userToUpdate.firstName;
        userToUpdate.lastName = formData.get('lastName') as string || undefined;
        userToUpdate.age = formData.get('age') ? parseInt(formData.get('age') as string, 10) : undefined;
        userToUpdate.address = formData.get('address') as string || undefined;
...
window.handleUserFilterChange = (event: Event) => {
    ...
    if (name === 'courseId') {
        ...
        // Reset enrollment status filter if course is set to 'all'
        if (userFilters.courseId === 'all') {
            userFilters.enrollmentStatus = 'all';
        }
    } else {
        (userFilters as any)[name as FilterKeys] = value;
    }
    render();
}
...
window.handleInitiatePixPayment = (paymentIds: number[]) => {
    ...
    pixModalContent = null; // show spinner
    render(); // show modal with spinner
    ...
    setTimeout(() => {
        ...
        render(); // re-render modal with content
...
window.handleExportDatabase = async () => {
    try {
        ...
        a.href = url;
        a.download = `sge_simulated_export_${new Date().toISOString().split('T')[0]}.json`;
...
// FIX: Added missing applyCardOrder and drag-and-drop handlers.
